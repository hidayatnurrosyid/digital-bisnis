...
<div class="max-w-7xl mx-auto px-6">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div class="col-span-3 text-center py-20 text-slate-400">
            <p class="text-xl font-bold">Tidak ada event tersedia.</p>
        </div>
    </div>
    @endforelse
</div>
...
{{-- ===== SECTION: MITRA KAMI ===== --}}
@if($partners->count() > 0)
<section class="bg-white border-t border-slate-100 py-16 px-6">
    <div class="max-w-7xl mx-auto">
        <div class="text-center mb-10">
            <p class="text-xs font-black uppercase tracking-widest text-indigo-400 mb-2">Didukung Oleh</p>
            <h2 class="text-3xl font-black text-slate-800">Mitra Kami</h2>
        </div>
        <div class="flex flex-wrap justify-center items-center gap-8">
            @foreach($partners as $partner)
            <div class="flex flex-col items-center gap-3 group">
                <div class="w-24 h-24 rounded-2xl border border-slate-100 shadow-sm overflow-hidden bg-slate-50 flex items-center justify-center group-hover:shadow-md transition">
                    <img
                ...
                <p class="text-xs font-bold text-slate-500 group-hover:text-indigo-600 transition text-center max-w-[100px]">{{ $partner->name }}</p>
            </div>
            @endforeach
        </div>
    </div>
